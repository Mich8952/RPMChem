...
class ModelComparatorSemantics:
    ...
    def compare(self, model_dir1, model_dir2):
        ...
        for i,set in enumerate(tqdm(self.dataset)):
            ...
            messages_1 = [
                {"role": "system", "content": sys_promt_van}, # to fit the training template
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(prompt1, str):
                        ...
                        prompt1_text = tokenizer1.decode(prompt1)
                    recovery_prompt1 = prompt1_text + text1.rstrip() + "\n\nSolution:\n"
                    recovery_completion1 = generate(
                        model_1,
                        tokenizer1,
                        prompt=recovery_prompt1,
                        verbose=False,
                        max_tokens=1000,
                        sampler=make_sampler(temp=0.7),
                    )
                    recovered_text1 = text1.rstrip() + "\n\nSolution:\n" + recovery_completion1.lstrip()
                    text1 = recovered_text1.split("Solution:\n", 1)[1]
